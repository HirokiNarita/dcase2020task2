...
# make graph
#############################################################################
def make_1D_graph(input, output, file_name):
...
# training function
def train_net(net, dataloaders_dict, criterion, optimizer, num_epochs, writer, machine_type):
    # make img outdir
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            for sample in tqdm(dataloaders_dict[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    loss, lower_bound, _, output = net(input, device)    # (batch_size,input(640)) 
                    kld = lower_bound[0]
                    reconst = lower_bound[1]
                    # 訓練時はbackprop
                    if phase == 'train':
                        loss.backward()
                        optimizer.step()
                ...
                klds[phase] += kld.item()
                reconsts[phase] += reconst.item()
            
                
                writer.add_scalar('{}_total_loss'.format(phase), loss.item())
                writer.add_scalar('{}_total_klds'.format(phase), kld.item())
                writer.add_scalar('{}_total_klds'.format(phase), reconst.item())
        ...
        epoch_klds['train'].append(klds['train'] / len(dataloaders_dict['train']))
        epoch_klds['valid'].append(klds['valid'] / len(dataloaders_dict['valid']))

        epoch_reconsts['train'].append(reconsts['train'] / len(dataloaders_dict['train']))
        epoch_reconsts['valid'].append(reconsts['valid'] / len(dataloaders_dict['valid']))
        ...
        if (epoch+1 % 10 == 0) or (epoch == 0):
            file_name_minibatch = img_out_dir + '/epoch_{}_minibatch.png'.format(epoch+1)
            make_1D_graph(input.cpu()[:,:], output.cpu()[:,:], file_name_minibatch)
            file_name = img_out_dir + '/epoch_{}.png'.format(epoch+1)
            make_1D_graph(input.cpu()[0,:], output.cpu()[0,:], file_name)
        
        valid_epoch_inputs.append(input)
        valid_epoch_output.append(output)
        # tensorboard
        writer.add_scalar('train_epoch_loss', epoch_losses['train'][-1])
        writer.add_scalar('valid_epoch_loss', epoch_losses['valid'][-1])
        writer.add_scalar('train_epoch_kld', epoch_klds['train'][-1])
        writer.add_scalar('valid_epoch_kld', epoch_klds['valid'][-1])
        writer.add_scalar('train_epoch_reconst', epoch_reconsts['train'][-1])
        writer.add_scalar('valid_epoch_reconst', epoch_reconsts['valid'][-1])
        
        scores = {'epoch_losses':epoch_losses, 'epoch_klds':epoch_klds, 'epoch_reconsts':epoch_reconsts}

    return {'socres':scores, 'valid_epoch_input':valid_epoch_inputs, 'valid_epoch_output':valid_epoch_output, 'model':net}
