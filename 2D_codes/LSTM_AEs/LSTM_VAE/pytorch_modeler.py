                ...
                for sample in tqdm(dataloaders_dict[phase]):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        if phase == 'train':
                            ...
                            reconst += output_dict['reconstruction'].item()
                            mu_loss += output_dict['mu_loss'].item()
                            sigma_loss += output_dict['sigma_loss'].item()
                # for visualization
                epoch_loss = tr_losses / len(dataloaders_dict['train'])
                epoch_KL = KL / len(dataloaders_dict['train'])
                epoch_reconst = reconst / len(dataloaders_dict['train'])
                epoch_mu_loss = mu_loss / len(dataloaders_dict['train'])
                epoch_sigma_loss = sigma_loss / len(dataloaders_dict['train'])

                tr_epoch_losses.append(epoch_loss)
                # tb
                writer.add_scalar("loss", epoch_loss, epoch+1)
                writer.add_scalar("KL", epoch_KL, epoch+1)
                writer.add_scalar("reconstruction", epoch_reconst, epoch+1)
                writer.add_scalar("mu_loss", epoch_mu_loss, epoch+1)
                writer.add_scalar("sigma_loss", epoch_sigma_loss, epoch+1)
            else:
                net.eval()
