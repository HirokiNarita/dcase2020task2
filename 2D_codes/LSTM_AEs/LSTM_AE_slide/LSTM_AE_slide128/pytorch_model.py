...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        hidden_n = hidden_n.reshape((batch_size, self.embedding_dim))
        return hidden_n
...
class LSTM_AutoEncoder(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax):
        ...
        self.encoder = Encoder(seq_len=128, n_features=mel_bins, embedding_dim=64)
        self.decoder = Decoder(seq_len=128, n_features=mel_bins, embedding_dim=64)
        #self.init_weight()
    ...
    def forward(self, input, device='cuda:0', mixup_lambda=None):
        ...
        #print(x.shape)
        #print(x.shape)
        x1,x2,x3,x4 = x[:, :128, :], x[:, 128:256, :], x[:, 256:384, :], x[:, 384:512, :]    # trim
