...
class DAGMM(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax, latent_size, mixture_size):
        ...
        self.estimation = Estimation_net(input_dim=latent_size+1, mid_dim=10, mixtures=mixture_size)
    ...
    def calc_reconstruction(self, x, x_dash):
        ...
        #x_dash = self.reconst_bn2(x_dash)
    ...
    def forward(self, input, device='cuda:0'):
        ...
        #plt.imshow(input_img[0,:,:].data.cpu().numpy(), aspect="auto")
        #plt.show()
        # network
        #x = self.bn0(x)
        enc = self.encoder(x)
        dec = torch.tanh(self.decoder(enc)) # torch.sigmoid_(
        #plt.imshow(dec[0,:,:].data.cpu().numpy(), aspect="auto")
        #plt.show()
        # calc reconstruction
