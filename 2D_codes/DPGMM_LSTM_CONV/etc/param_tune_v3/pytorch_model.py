...
class DAGMM(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax, latent_size, mixture_size):
        ...
        self.metaDense = metaDense(input_dim=1024, mid_dim=512, comp_dim=self.latent_size)

        self.estimation = Estimation_net(input_dim=self.latent_size+1, mid_dim=int((self.latent_size+1)/2), mixtures=self.mixture_size)
        self.bn3 = nn.BatchNorm1d(self.latent_size+1)
        self.init_weight()
    ...
    def calc_reconstruction(self, x, x_dash):
        ...
        #rec_euclidean = F.mse_loss(x_dash, x)
        #print(rec_euclidean)
        #print(rec_cosine)
        #print(rec_euclidean)
    ...
    def forward(self, input, device='cuda:0'):
        ...
        #print(dec.shape)
        # calc reconstruction
        rec_euclidean, _ = self.calc_reconstruction(input_img, dec)
