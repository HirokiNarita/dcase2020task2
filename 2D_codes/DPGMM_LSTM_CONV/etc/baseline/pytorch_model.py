...
class Encoder(nn.Module):
    def __init__(self, seq_len, n_features, hidden_dim=128):
        ...
        self.conv1 = ConvBlock(in_channels=2, out_channels=32)
...
class Decoder(nn.Module):
    def __init__(self, seq_len, n_features, hidden_dim=128):
        ...
        self.deconv6 = deConvBlock(in_channels=64, out_channels=32)
        self.deconv7 = deConvBlock(in_channels=32, out_channels=1)
...
class metaDense(nn.Module):
    def __init__(self, input_dim, mid_dim, comp_dim):
        ...
        self.fc2 = nn.Linear(self.mid_dim, self.comp_dim)
...
class Estimation_net(nn.Module):
    def __init__(self, input_dim, mid_dim, mixtures):
        ...
        self.fc2 = nn.Linear(self.input_dim, self.mid_dim)
        self.bn2 = nn.BatchNorm1d(self.input_dim)
        self.fc3 = nn.Linear(self.input_dim, self.mid_dim)
        self.bn3 = nn.BatchNorm1d(self.mid_dim)
        self.fc4 = nn.Linear(self.mid_dim, self.mixtures)
        self.act1 = nn.PReLU()
        self.act2 = nn.PReLU()
        self.act3 = nn.PReLU()
        self.init_weight()
...
class DAGMM(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax, latent_size, mixture_size):
        ...
        self.metaDense = metaDense(input_dim=512, mid_dim=512, comp_dim=latent_size)

        self.estimation = Estimation_net(input_dim=latent_size+1, mid_dim=latent_size+1, mixtures=mixture_size)
        self.init_weight()
