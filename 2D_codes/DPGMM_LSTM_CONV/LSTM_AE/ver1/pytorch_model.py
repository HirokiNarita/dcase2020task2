...
class FC_block(nn.Module):
    def __init__(self, in_features, out_features, dim1_shape):
        
        super(FC_block, self).__init__()
        
        self.fc1 = nn.Linear(in_features, out_features)
        self.bn1 = nn.BatchNorm1d(dim1_shape)
        self.act1 = nn.PReLU()
        self.init_weight()
...
class Encoder(nn.Module):
    def __init__(self, seq_len, n_features, hidden_dim=128, comp_dim=5):
        ...
        self.fc1 = FC_block(128, 64, 64)
        self.fc2 = FC_block(64, 32, 64)
        self.fc3 = FC_block(32, 1, 64)

        self.fc4 = FC_block(64, 32, 32)
        self.fc5 = FC_block(32, self.comp_dim, self.comp_dim)

        #self.fc3 = FC_block(in_features=32, out_features=self.comp_dim)
    ...
    def forward(self, x):
        ...
        x, _ = self.lstm(x)
        x = x.view(batch_size, 64, 128) # (batch_size, seq_len, hidden_dim)
        #print(x.shape)
        x = self.fc1(x)
        x = self.fc2(x)
        x = self.fc3(x)
...
class Decoder(nn.Module):
    def __init__(self, seq_len, n_features, hidden_dim=128, comp_dim=5):
        ...
        #self.lstm_out_features = 2*hidden_dim    #(forward+backward)
        #self.input_shape = int(self.n_features * self.seq_len)
        self.fc1 = FC_block(self.comp_dim, 32, 32)
        self.fc2 = FC_block(32, 64, 64)
        
        self.fc3 = FC_block(1, 32, 64)
        self.fc4 = FC_block(32, 64, 64)
