...
def train_net(net, dataloaders_dict, criterion, optimizer, num_epochs, writer):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'valid']:
            if phase == 'train':
                ...
                for sample in tqdm(dataloaders_dict[phase]):
                    ...
                    input = to_var(input)
                    net, total_loss, sample_energy, recon_error, cov_diag = dagmm_step(net, input, optimizer, device)
        ...
        logger.info('Epoch {}/{}:train_loss:{:.6f}, tr_eng:{:.6f}, tr_covd:{:.6f}, val_AUC:{:.6f}, val_pAUC:{:.6f}'.format(epoch+1,
                                                                                                                           num_epochs,
                                                                                                                           tr_losses,
                                                                                                                           tr_eng,
                                                                                                                           tr_covd,
...
def dagmm_step(net, input_data, optimizer, device):
    net.train()
    #optimizer.zero_grad()
