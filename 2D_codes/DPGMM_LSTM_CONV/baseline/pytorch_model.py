...
class DAGMM(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax):
        ...
        self.metaDense = metaDense(input_dim=512, mid_dim=256, comp_dim=32)

        self.decoder = Decoder(seq_len=128, n_features=128, hidden_dim=128)

        self.estimation = Estimation_net(input_dim=32+1, mid_dim=10, mixtures=5)
        self.init_weight()
    ...
    def compute_energy(self, z, phi=None, mu=None, cov=None, size_average=True):
        ...
        for i in range(k):
            # K x D x D
            cov_k = cov[i] + to_var(torch.eye(D)*1e-1)
