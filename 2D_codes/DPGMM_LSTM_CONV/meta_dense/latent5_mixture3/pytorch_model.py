...
class deConvBlock(nn.Module):
    ...
    def forward(self, input, out=False):
        x = input
        if out == True:
            x = torch.sigmoid(self.bn1(self.deconv1(x)))
        else:
            x = self.act1(self.bn1(self.deconv1(x)))
            #x = self.bn1(torch.tanh_(self.deconv1(x)))
        return x
...
class Meta_dense(nn.Module):
    def __init__(self, input_dim, mid_dim, comp_dim):

        super(Meta_dense, self).__init__()
        ...
        self.enc_bn3 = nn.BatchNorm1d(self.comp_dim)
    ...
    def init_weight(self):
        ...
        init_bn(self.enc_bn3)
    
    def forward(self, x):
        x = self.act1(self.enc_bn1(self.enc_fc1(x)))
        x = self.act1(self.enc_bn2(self.enc_fc2(x)))
        comp = torch.sigmoid(self.enc_bn3(self.enc_fc3(x)))
        return comp
...
class DAGMM(nn.Module):
    ...
    def forward(self, input, device='cuda:0'):
        ...
        return {'x':input_img, 'x_hat':dec, 'z_c':z_c, 'z':z, 'gamma':gamma, 'enc':enc}
