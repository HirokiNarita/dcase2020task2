...
class metaAE(nn.Module):
    def __init__(self, input_dim, mid_dim, comp_dim):
        ...
        self.enc_bn0 = nn.BatchNorm1d(self.input_dim)
        self.enc_fc1 = nn.Linear(self.input_dim, 512)

        self.enc_bn1 = nn.BatchNorm1d(512)
        self.enc_fc2 = nn.Linear(512, 128)
        
        #self.enc_dim = int(self.enc_dim / 2)
        self.enc_bn2 = nn.BatchNorm1d(128)
        self.enc_fc3 = nn.Linear(128, self.comp_dim)
        
        # decoder
        self.dec_bn1 = nn.BatchNorm1d(self.comp_dim)
        self.dec_fc1 = nn.Linear(self.comp_dim, 128)
        
        self.dec_bn2 = nn.BatchNorm1d(128)
        self.dec_fc2 = nn.Linear(128, 512)

        self.dec_bn3 = nn.BatchNorm1d(512)
        self.dec_fc3 = nn.Linear(512, self.input_dim)
    ...
    def init_weight(self):
        init_layer(self.enc_fc1)
        init_layer(self.enc_fc2)
        init_layer(self.enc_fc3)
        init_bn(self.enc_bn0)
        init_bn(self.enc_bn1)
        init_bn(self.enc_bn2)

        init_layer(self.dec_fc1)
        init_layer(self.dec_fc2)
        init_layer(self.dec_fc3)
        init_bn(self.dec_bn1)
        init_bn(self.dec_bn2)
        init_bn(self.dec_bn3)
        
    
    def forward(self, x):
        x = self.enc_bn0(x)
        x = F.leaky_relu_(self.enc_fc1(x))
        x = self.enc_bn1(x)
        x = F.leaky_relu_(self.enc_fc2(x))
        #x = F.dropout(x, p=0.2, training=self.training)
        x = self.enc_bn2(x)
        #print(x.shape)
        comp = torch.tanh_(self.enc_fc3(x))

        x = self.dec_bn1(comp)
        x = F.leaky_relu_(self.dec_fc1(x))
        x = self.dec_bn2(x)
        x = F.leaky_relu_(self.dec_fc2(x))
        x = self.dec_bn3(x)
        x = torch.tanh_(self.dec_fc3(x))
        return comp, x
...
class DAGMM(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax, latent_size, mixture_size):
        ...
        self.metaAE = metaAE(input_dim=2048, mid_dim=512, comp_dim=latent_size)
        #self.reconst_bn1 = nn.BatchNorm1d(1)
        #self.reconst_bn2 = nn.BatchNorm1d(1)
        self.estimation = Estimation_net(input_dim=latent_size+2, mid_dim=int((latent_size+1)/2), mixtures=mixture_size)
        self.bn1 = nn.BatchNorm1d(latent_size+2)
        self.init_weight()
    ...
    def calc_reconstruction(self, x, x_dash):
        ...
        rec_euclidean = self.relative_euclidean_distance(x, x_dash)
        #print(rec_cosine)
        #print(rec_euclidean)
        return rec_euclidean.unsqueeze(-1) # unsqueezeを要確認
    ...
    def forward(self, input, device='cuda:0'):
        ...
        z_c, meta_hat = self.metaAE(enc)
        meta_euclidean = self.relative_euclidean_distance(enc, meta_hat).unsqueeze(-1)
        meta_loss = F.mse_loss(meta_hat, enc)
        z = torch.cat([z_c, meta_euclidean, rec_euclidean], dim=1) # unuse cosine_similarity
        #z = self.bn1(z)
        #print(z)
        # estimation net
        gamma = self.estimation(z)
        return {'x':input_img, 'x_hat':dec, 'z_c':z_c, 'z':z, 'gamma':gamma, 'enc':enc, 'meta_loss':meta_loss}
    ...
    def loss_function(self, x, x_hat, z, gamma, meta_loss, lambda_energy, lambda_cov_diag):
        ...
        loss = recon_error + meta_loss + lambda_energy * sample_energy + lambda_cov_diag * cov_diag
