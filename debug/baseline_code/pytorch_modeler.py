    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            for sample in tqdm(dataloaders_dict[phase]):
                ...
                # フレームごとに学習させていくループ
                # optimizerの初期化
                optimizer.zero_grad()　　　 ### 変更 ###
                loss = 0                    ### 変更 ###
                for image_num in range(features.shape[0]):
                    # minibatchから2Dデータを取り出す
                    x = features[image_num,:]   # shape(309,640)
                    # 順伝播(forward)
                    with torch.set_grad_enabled(phase == 'train'):
                        x = x.to(device, dtype=torch.float32)
                        outputs = net(x)                 # (row(309),input(640)) 
                        loss += criterion(outputs, x)    ### 変更 ###
